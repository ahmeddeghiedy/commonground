  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/workspaces", {
        ...
        body: JSON.stringify({ name, destination, nights, organizerName }),
      });
  ...
  return (
    <div className="cg-overlay cg-overlay--guide" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <section className="cg-collab-dialog" role="dialog" aria-modal="true" aria-labelledby="create-trip-title">
        ...
        <p className="cg-dialog-lead">Start with your details, then invite up to 11 more travelers with private links. No accounts or passwords needed.</p>
